...
void criaTarefa(
    struct lista *lt) { // FUNÇÃO UTILIZADA PARA CRIAR UMA NOVA TAREFA QUE
                        // RECEBE UM PONTEIRO PARA O STRUCT.
  ...
  printf("ESTADO DA TAREFA (COMPLETO, EM ANDAMENTO, NÃO INICIADO) [0, 1, 2 "
         "resopectivamente]: ");
  scanf("%d", &estado);
...
void listaTarefas(int tam,
                  struct lista *lt) { // FUNÇÃO UTILIZADA PARA LISTAR AS TAREFAS
  for (int i = 0; i < tam; i++) {
    printf("TAREFA %d\n", i + 1);
    printf("PRIORIDADE: %d\n", lt[i].prioridade);
    printf("CATEGORIA: %s\n", lt[i].categoria);
...
void atualizaTarefa(int tam, struct lista *lista, int index) {
  ...
  printf("\nQUAL CAMPO DESEJA ALTERAR? (Prioridade, categoria, descrição, "
  ...
  case 3:
    ...
    break;
  case 4:
    printf("\nESTADO DA TAREFA (COMPLETO, EM ANDAMENTO, NÃO INICIADO) [0, 1, 2 "
           "respectivamente]: ");
    scanf("%d", &estado_novo);
...
int tamanho(struct lista
                *lt) { // FUNÇÃO QUE RETORNA O TAMANHO ATUAL DA LISTA DE TAREFAS
  FILE *strArquivo = fopen("ListaDeTarefas", "rb");
  int aux = 0;
  if (strArquivo) {
    // CONTA O NÚMERO DE TAREFAS (STRUCTS) PRESENTES NO ARQUIVO ATRAVÉS DA
    // VARIÁVEL 'aux'
...
void deletaTarefa(int *tam, struct lista *lt,
                  int posTarefa) { // FUNÇÃO PARA DELETAR UMA TAREFA.
  if (posTarefa < 1 ||
      posTarefa > *tam) { // AQUI É ESCOLHIDA A POSIÇÃO DA TAREFA QUE O USUÁRIO
                          // DESEJA DELETAR NO ARRAY DA LISTA
    printf("OPCAO INVALIDA. DIGITE UM NUMERODE 1 A %d.\n", *tam);
    return;
  }

  for (int i = posTarefa - 1; i < *tam - 1;
       i++) { // SUBSTITUI AS INFORMAÇÕES DAS POSIÇÕES PELA POSIÇÃO SEGUINTE
              // (MOVE AS OUTRAS TAREFAS PARA "TRÁS" PARA PREENCHER O "BURACO"
    lt[i] = lt[i + 1];
  }
...
void salvaTexto(int tam, struct lista *lista, int prioridade) {
  ...
  printf("Digite o nome do arquivo para salvar (insira '.txt' ao final do "
         "nome): ");
  scanf("%s", nome);
...
void filtraCategoriaPrioridade(int tam, struct lista *lista, int prioridade,
                               char categoria[300]) {
  qsort(lista, tam, sizeof(struct lista), compara_sort);
  ...
  for (int i = 0; i < tam; i++) {
    ...
    if ((prioridade == lista[i].prioridade) &&
        (strcmp(categoria, lista[i].categoria) == 0)) {
      printf("TAREFA %d\n", i + 1);
      printf("PRIORIDADE: %d\n", lista[i].prioridade);
      printf("CATEGORIA: %s\n", lista[i].categoria);
      printf("DESCRICAO: %s\n", lista[i].descricao);
...
void salvaCategoriaPrioeidade(int tam, struct lista *lista, int prioridade,
                              char categoria[300]) {
  char nome[300];
...
void salvar(
    int tam,
    struct lista *lt) { // FUNÇÃO PARA ESCREVER A LISTA DE TAREFAS NO ARQUIVO.
