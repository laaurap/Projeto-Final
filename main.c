...
int main() {
  ...
  tam = tamanho(lt); // A VARIÁVEL 'tam' ARMAZENA O TAMANHO (NÚMERO DE TAREFAS)
                     // ATUAL DA LISTA.
  ...
  while (1) {
    ...
    switch (opcao) {
    case (1):
      if (tam == 100) { // VERIFICA SE A LISTA ESTÁ "CHEIA" (100 TAREFAS) E
                        // EXIBE UMA MENSAGEM DE ERRO CASO ESTIVER.
        printf(
            "LIMITE DE TAREFAS ATINGIDO. DELETE UMA TAREFA PARA CRIAR MAIS.\n");
      }
      printf("\n--- CRIAR TAREFA ---\n");
      criaTarefa(&lt[tam]);
      tam++; // AUMENTA O TAMAHO DA LISTA DE TAREFAS EM 1, POIS FOI REGISTRADA
             // UMA NOVA TAREFA.
      break;
    case (2):
      printf("\n--- LISTAR TAREFAS ---\n");
    ...
    case 6:
      ...
      printf("ESTADO (Completo, Em andamento, Não iniciado) [0, 1, 2 "
             "RESPECTIVAMENTE]: ");
    ...
    case 10:
      ...
      filtraCategoriaPrioridade(tam, lt, prioridade_categoria,
    ...
    case (0):
      printf("\nVOCE ESCOLHEU SAIR.\n");
      salvar(tam, lt);
      auxBool = 1; // MUDA O VALOR DE 'auxBool' PARA 1 PARA PARAR A ITERAÇÃO DO
                   // 'while'
      break;
    default:
      printf(
